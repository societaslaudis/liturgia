...
$inputFileName = "c:/liturgia/celebrations_mobiles.xlsx";
...
$objReader->setLoadSheetsOnly("cycle_avent_france");
$objPHPExcel = $objReader->load($inputFileName);
$sheetDataTemporal = $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);

foreach ($sheetDataTemporal as $line) {
	print "\r\n".$line['B']." ".$line['C']." ".substr_compare($line['B'], "perannum_", 0, 8);
	if(
		(substr_compare($line['B'], "perannum_", 0, 8)==0)
		&&
		($line['C'])
		&&
		(@substr_compare($line['C'], "Dominica", 0, 8)==0)
		){
			print" OK";
			
			$dxml=simplexml_load_file("sources/propres/".no_accent($line['C']).".xml");
			$c_temporal=$dxml->xpath("oratio/@id");
			$collecte_temporal=$c_temporal[0];
			$heb=substr($line['B'],0,10);
			$hebb=explode("-",$line['B']);
			$heb=$hebb[0];
			for ($j=1;$j<8;$j++) {
				$thisheb=$heb."-".$j;
				print"\r\n ".$thisheb." -> ".$collecte_temporal;
				$thisxmltemp=simplexml_load_file("sources/propres/".$thisheb.".xml");
				if($thisxmltemp){
					$ct=$thisxmltemp->addChild('collecte_temporal');
					$ct->addAttribute('id',$collecte_temporal);	
					$thisxmltemp->asXML("sources/propres/".$thisheb.".xml");
				} 
